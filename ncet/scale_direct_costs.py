...
def scale_direct_costs(base, scaling_table, plant_characteristics, scalars_dict):
	...
	for account in scaling_table.index:
		...
		dfNP.loc[idx, (costs + hours)] *= scaling_table.at[account, 'Scaling Factor'] * scaling_table.at[account, 'Multipliers']
	...
	dfNP['Total Cost'] = dfNP['Site Material Cost'].values + dfNP['Site Labor Cost'].values + dfNP['Factory Equipment Cost'].values
