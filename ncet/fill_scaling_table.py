...
def fill_scaling_table(path, fname, base, scalars_dict, scaling_table = None):
	...
	if scaling_table is None:
		scaling_table = pd.read_csv(path+"input_scaling_exponents.csv", header=0, index_col='Account')
	else:
	...
	plant_characteristics = pd.read_excel(path+fname, sheet_name='PlantCharacteristics', header=None, skiprows=[0], index_col=0).to_dict()[1]
	...
	plant_characteristics['sc1_concrete'] = 0 # Seismic Category 1 concrete Volume tally
	concrete = 0
	bv_accounts_225     = ['A.212.', 'A.213.', 'A.215.', 'A.216.', 'A.217.']
	bv_225 = 0

	df21 = pd.read_excel(path+fname, header=0, sheet_name='21-Structures&Improvements', skiprows=[0], index_col='Account')
	df22 = pd.read_excel(path+fname, header=0, sheet_name='22-ReactorEquipment', skiprows=[0], index_col='Account')
	df23 = pd.read_excel(path+fname, header=0, sheet_name='23-TurbineEquipment', skiprows=[0], index_col='Account')
	df24 = pd.read_excel(path+fname, header=0, sheet_name='24-ElectricalEquipment', skiprows=[0], index_col='Account')
	df25 = pd.read_excel(path+fname, header=0, sheet_name='25-MiscEquipment', skiprows=[0], index_col='Account')
	df26 = pd.read_excel(path+fname, header=0, sheet_name='26-HeatRejectionSystem', skiprows=[0], index_col='Account')
	...
	if any(df21['Rebar density']!='Default'):
		plant_characteristics['Rebar table'] = df21.loc[df21['Rebar density']!='Default', 'Rebar density'].to_dict()
	...
	for account in accounts:
		...
		if aux['Method']=='Detailed (EEDB based)':
			...
			if aux['Inside?'] != 'None':
				...
				if in_or_out == 'Inside':
					inside_dict[inside_acct] = [account, subArea, subVol, superArea, superVol, bv] 
				elif in_or_out == 'Outside':
					in_account, in_subArea, in_subVol, in_superArea, in_superVol, in_bv = inside_dict[account]
					subArea -= in_subArea
					subVol  -= in_subVol
					bv      -= in_bv
			print("		Superstructure volume: {:.0F}, area: {:.0F}".format(superVol, superArea))
			...
			scaling_table.loc[(idx & ipow), 'New Base Unit Value'] = plant_characteristics['Total Plant Thermal Power (MWt)']
			...
			if aux['Name']=='Containment Liner':
				...
				if aux['Superstructure type']=='Stainless steel vessel':
					...
					mass = 8000.*(superVol + subVol) # 8000 kg/m^3 is the density of stainless steel
					print("		Mass of containment vessel: {:.0F}".format(mass))
					scaling_table.at[account, 'New Base Unit Value'] = mass
					scaling_table.at[account, 'Multipliers']	= 2.3 #stainless more than carbon steel	
					scaling_table.at[account, 'Count per plant'] = plant_characteristics['Number of Reactors']
					...
					scaling_table.at[account, 'Factory Equipment Cost Mult']  *= scalars_dict['212.15 Factory cost mult']
					scaling_table.at[account, 'Site Labor Hours Mult']        *= scalars_dict['212.15 Labor hours mult']
					scaling_table.at[account, 'Site Labor Cost Mult']         *= scalars_dict['212.15 Labor cost mult']
					scaling_table.at[account, 'Site Material Cost Mult']      *= scalars_dict['212.15 Material cost mult']
					plant_characteristics['Containment type'] = ['Standalone steel building']
			...
			scaling_table.loc[idx, 'New Base Unit Value'] = plant_characteristics['Total Plant Thermal Power (MWt)']
			
		elif aux['Method']=='RX power scaling':
			idx = scaling_table.index.str.match(account)
			scaling_table.loc[idx, 'Option'] = 2
			scaling_table.at[idx, 'New Base Unit Value'] = plant_characteristics['Total Plant Thermal Power (MWt)'] / plant_characteristics['Number of Reactors']
			...
			scaling_table.at[idx, 'New Base Unit Value'] = df21.loc[account, 'Direct cost per RX (2018 USD)']
	...
	scaling_table.loc[idx_PPS, 'New Base Unit Value'] = plant_characteristics['Total Plant Thermal Power (MWt)']
	
	idx_EPS = df_big.index[df_big['Method']=='Plant electric power scaling']
	scaling_table.loc[idx_EPS, 'Option'] = 2
	scaling_table.loc[idx_EPS, 'New Base Unit Value'] = plant_characteristics['Net Electrical Power (MWe)']
	
	idx_TEPS = df_big.index[df_big['Method']=='Turbine electric power scaling']
	scaling_table.loc[idx_TEPS, 'Option'] = 2
	scaling_table.loc[idx_TEPS, 'New Base Unit Value'] = plant_characteristics['Net Electrical Power (MWe)']
	scaling_table.loc[idx_TEPS, 'Count per plant']     = plant_characteristics['Number of Reactors']
	
	idx_RPS = df_big.index[df_big['Method']=='RX power scaling']
	scaling_table.loc[idx_RPS, 'Option'] = 2
	scaling_table.loc[idx_RPS, 'New Base Unit Value'] = plant_characteristics['Total Plant Thermal Power (MWt)'] / plant_characteristics['Number of Reactors']
	scaling_table.loc[idx_RPS, 'Count per plant']     = plant_characteristics['Number of Reactors']
	...
	scaling_table.loc[idx_D, 'Count per plant']     = df_big.loc[idx_D, 'Count per plant (DI)']
	...
	scaling_table.loc[idx_Dp, 'Count per plant'] = df_big.loc[idx_Dp, 'Count per plant (DI)']
	...
	scaling_table.loc[idx_Dce, 'Count per plant'] = df_big.loc[idx_Dce, 'Count per plant (DI)']

	idx_DCI = df_big.index[df_big['Method']=='Direct cost input']
	scaling_table.loc[idx_DCI, 'Option'] = 3
	scaling_table.loc[idx_DCI, 'New Base Unit Value'] = df_big.loc[idx_DCI, 'Direct cost per RX (2018 USD)']
	scaling_table.loc[idx_DCI, 'Count per plant'] = df_big.loc[idx_DCI, 'Count per plant (DCI)']
	...
	scaling_table.loc[accounts_1, 'Scaling Factor'] = (scaling_table.loc[accounts_1, 'New Base Unit Value'] / scaling_table.loc[accounts_1, 'EEDB Base Unit Value 1'])** scaling_table.loc[accounts_1, 'Option 1 Exponent']
	scaling_table.loc[accounts_2, 'Scaling Factor'] = (scaling_table.loc[accounts_2, 'New Base Unit Value'] / scaling_table.loc[accounts_2, 'EEDB Base Unit Value 2'])** scaling_table.loc[accounts_2, 'Option 2 Exponent']
	scaling_table.loc[accounts_3, 'Scaling Factor'] = (scaling_table.loc[accounts_3, 'New Base Unit Value'] / scaling_table.loc[accounts_3, 'EEDB Base Unit Value 3'])
	...
	for account in accounts_0:
		if not isinstance(scaling_table.at[account, 'Option 0 Formula'], list):
			varz = scaling_table.at[account, 'Option 0 Formula'].replace('[','').replace(']','').split(',')
		else:
			varz = scaling_table.at[account, 'Option 0 Formula']
		varz = [float(x) for x in varz]
		if len(varz)==4:
			scaling_table.at[account, 'Scaling Factor'] = (varz[0] + varz[1] * scaling_table.at[account, 'New Base Unit Value'] **varz[2]) * varz[3] / scaling_table.at[account, 'EEDB Base Unit Value 3']
		elif len(varz)==1:
			scaling_table.at[account, 'Scaling Factor'] = varz[0] * scaling_table.at[account, 'New Base Unit Value'] / scaling_table.at[account, 'EEDB Base Unit Value 3']
	...
	plant_characteristics['sc1_concrete'] += scaling_table.at['A.212.140', 'Scaling Factor']*8000/1.1**3 # 8000 CY of interior concrete in EEDB
	concrete += scaling_table.at['A.212.140', 'Scaling Factor']*8000/1.1**3 # 8000 CY of interior concrete in EEDB
