...
def get_bldg(aux, dimensions):
    ...
    superArea = {
        ...
        "Cylinder w/ dome": lambda d, h, u: 4 * np.pi * d ** 2 / 4 / 2
        + np.pi * d * h,  # u for unused
    }[shape](*dimensions)
    ...
    bv = {
        ...
        "Cylinder w/ dome": lambda d, h, u: np.pi * 1 / 6 * d ** 3 / 2
        + np.pi * d ** 2 / 4 * h,  # u for unused
    }[shape](*dimensions)
...
def eval_bldg(portions, aux):
    if portions == 1:
        dimensions = (
            aux["Dimensions (meters)"].replace("[", "").replace("]", "").split(",")
        )
        tot_subArea, tot_subVol, tot_superArea, tot_superVol, tot_bv = get_bldg(
            aux, dimensions
        )
    else:
        tot_subArea = 0
