...
def special_cases(dfNewPlant, plant_characteristics, scalars_dict):
	...
	for i, account in enumerate(accounts):
		...
		unit_mat_cost  = dfNewPlant.loc[idx_rebar, 'Site Material Cost'] / dfNewPlant.loc[idx_rebar, 'Site Quantity']
		unit_lab_cost  = dfNewPlant.loc[idx_rebar, 'Site Labor Cost']    / dfNewPlant.loc[idx_rebar, 'Site Quantity']
		unit_lab_hours = dfNewPlant.loc[idx_rebar, 'Site Labor Hours']   / dfNewPlant.loc[idx_rebar, 'Site Quantity']
		dfNewPlant.loc[idx_rebar,'Site Quantity'] *= reduction # % reduction the rebar density
		dfNewPlant.loc[idx_rebar, 'Site Material Cost'] = dfNewPlant.loc[idx_rebar,'Site Quantity'] * unit_mat_cost * cost_mult # % material cost increase
		dfNewPlant.loc[idx_rebar, 'Site Labor Cost']    = dfNewPlant.loc[idx_rebar,'Site Quantity'] * unit_lab_cost
		dfNewPlant.loc[idx_rebar, 'Site Labor Hours']   = dfNewPlant.loc[idx_rebar,'Site Quantity'] * unit_lab_hours
		dfNewPlant.loc[idx_rebar, 'Total Cost']         = dfNewPlant.loc[idx_rebar,'Site Labor Cost'] + dfNewPlant.loc[idx_rebar,'Site Material Cost']

		dfNewPlant.loc[idx_cadwelds,(costs + hours)] *= reduction # % reduction the rebar density
		dfNewPlant.loc[idx_cadwelds, 'Total Cost']    = dfNewPlant.loc[idx_cadwelds,'Site Labor Cost'] + dfNewPlant.loc[idx_cadwelds,'Site Material Cost']
	...
	lab_rates = pd.read_csv('input_LaborIndices.csv', dtype={'1986':float,'2018':float,'Index':float, 'Base Rate':float})
	lab_rates.set_index('Craft', inplace=True)
	operating_eng_rate = lab_rates.loc['Operating Engineers', 'Base Rate'] * lab_rates.loc['Operating Engineers', 'Index']
	ironworker_rate    = lab_rates.loc['Ironworkers', 'Base Rate'] * lab_rates.loc['Ironworkers', 'Index']

	aux = {"One sided": [scalars_dict['SPC rebar reduction mult']/2, scalars_dict['SPC steel cost escalation'] ],
		   "Two sided": [scalars_dict['SPC rebar reduction mult']  , scalars_dict['SPC steel cost escalation'] ]}
	for i, account in enumerate(spc_accounts):
		...
		idx_spc = idx & (dfNewPlant['Category2']!='Interior Concrete') & (dfNewPlant['Category2']!='Substructure Concrete') # don't apply the steel plate composite to interior concrete or foundation
		...
		dfNewPlant.loc[idx_rebar, 'Site Material Cost'] *= cost_mult #rebar cost is 1.48 times higher
		dfNewPlant.loc[idx_formwork, (costs + hours)]   *= work_reduction  #formwork becomes half or fully free
		dfNewPlant.loc[idx_cadwelds, (costs + hours)]    = 0.0  #no cadwelding with spc

		newPlant_superArea = plant_characteristics['SPC Area'][i]
		dfNewPlant.loc[idx_rebar, ['Site Labor Cost', 'Site Labor Hours']] = 0.0 #first rebar install becomes free
		lift_time = scalars_dict['SPC operating engineer lift time (per 30 m2)']
		dfNewPlant.loc[idx_rebar, 'Site Labor Hours'] =  newPlant_superArea/30.2 * lift_time #each section is 30.2 m^2 and takes ~8 hours to lift
		dfNewPlant.loc[idx_rebar, 'Site Labor Cost'] = dfNewPlant.loc[idx_rebar, 'Site Labor Hours'] * operating_eng_rate

		weld_time = scalars_dict['SPC weld time (per 30 m2)']
		weld_hours =  newPlant_superArea/30.2 * weld_time * (1-work_reduction) #each section is 30.2 m^2 and takes ~87 hours to weld, half the welding for the one sided case
	...
	if 'Rebar table' in plant_characteristics:
		...
		for bldg in rebar_dict:
			if bldg=='A.212.':
				...
				densities = rebar_dict[bldg].replace('[','').replace(']','').split(',')
				densities = [float(x) for x in densities]
				densities.insert(1, densities[1]) # combining shell and dome for superstructure
				for i, account in enumerate(accounts):
					dfNewPlant.loc[account, (costs+hours)] *= densities[i]/defaults[i]
	...
	if plant_characteristics['Containment type'] == 'Steel vessel':
		...
		idx_rcp   = (dfNewPlant['Account Description'] == 'Reactor Cavity Liner Plate') & idx
		idx_mse   = (dfNewPlant['Account Description'] == 'Major Support Embedments') & idx
	...
	if plant_characteristics['RPV thickness (m)'] <= .11:
		#EPRI can E-beam weld (and do other things to) your vessel reducing cost ~40%
		dfNewPlant.loc['A.221.12', (costs + hours)] *= scalars_dict['E-beam weld cost reduction mult']

	if plant_characteristics['Integral PWR']: # no vessel cost for an integrated steam generator/pressurizer/vessel
		dfNewPlant.loc['A.222.13', (costs + hours)] *= scalars_dict['222.13 Steam generators reduction mult']  
		dfNewPlant.loc['A.222.14', (costs + hours)] *= scalars_dict['222.14 Pressurizer reduction mult']  
