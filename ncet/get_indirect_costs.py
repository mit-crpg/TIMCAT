...
def get_indirect_costs(dfNewPlant, plant_characteristics, learning_rate, scalars_dict):
	...
	construction_months = plant_characteristics['Construction duration (months)'] * learning_rate
	...
	NNC_indirect_cost     = {'Factory Equipment Cost':0,      'Site Labor Hours': 0,      'Site Labor Cost': 0,       'Site Material Cost': 0,       'Total Cost':0}
	NQA_indirect_cost     = {'Factory Equipment Cost':0,      'Site Labor Hours': 0,      'Site Labor Cost': 0,       'Site Material Cost': 0,       'Total Cost':0}
	np_indirect_cost      = {'Factory Equipment Cost':0,      'Site Labor Hours': 0,      'Site Labor Cost': 0,       'Site Material Cost': 0,       'Total Cost':0}
	indirect_multiplier   = {'Factory Equipment Cost':3.661,  'Site Labor Hours': 0.360,  'Site Labor Cost': 0.360,   'Site Material Cost': 0.785}
	...
	"Pre compute values"
	total_direct_labor_hours = dfNewPlant.loc[(root_idx & direct_idxNP), 'Site Labor Hours'].sum()
	avgNumWorkersNewPlant = total_direct_labor_hours / construction_months / 160 #160 working hours in a month
	mult_workers          = np.max([1, avgNumWorkersNewPlant/1058]) # 1058 comes from the EEDB average 12.1 million hours over 72 months, setting the max =1, assuming the BE plant was peak efficiency in staffing
	mult_constructionTime = np.max([1, construction_months/72]) #the PWR12 better experience from EEDB took 72 months to build, assume there isnt a gain for shorter construction here
	...
	indirect_multiplier['Site Material Cost']     = indirect_multiplier['Site Material Cost'] * mult_workers
	indirect_multiplier['Factory Equipment Cost'] = indirect_multiplier['Factory Equipment Cost'] * mult_constructionTime
	...
	NQA_indirect_cost['Factory Equipment Cost'] = indirect_multiplier['Factory Equipment Cost'] * NQA_indirect_cost['Site Labor Cost']
	...
	"Scaling the indirect factory costs and total cost" #have to do this after the labor hours scaling
	NNC_indirect_cost['Factory Equipment Cost'] = indirect_multiplier['Factory Equipment Cost'] * NNC_indirect_cost['Site Labor Cost']
	NNC_indirect_cost['Total Cost'] = sum(NNC_indirect_cost.values())

	#----------------- Split costs ----------------
	idx = (root_idx & direct_idxNP & NQA_idx)
	NQA_direct_sum = dfNewPlant.loc[idx, 'Factory Equipment Cost'].sum() + dfNewPlant.loc[idx, 'Site Labor Cost'].sum() + dfNewPlant.loc[idx, 'Site Material Cost'].sum()
	NQA_total_max = 1.49 * NQA_direct_sum

	idx = (root_idx & direct_idxNP & ~NQA_idx)
	NNC_direct_sum = dfNewPlant.loc[idx, 'Factory Equipment Cost'].sum() + dfNewPlant.loc[idx, 'Site Labor Cost'].sum() + dfNewPlant.loc[idx, 'Site Material Cost'].sum()
	...
	if NQA_diff > -NNC_diff: #Essentially do the total shifts in cost make sense, and should we correct for over/under expensive indirect costs
		NQA_extra = -NNC_diff
	else:
		NQA_extra = NQA_diff
