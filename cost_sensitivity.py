        ...
        if option==0:
            for idx in df_inputs.index:
                if df_inputs['Option 0'].iloc[idx] is not np.nan:
                    varz = df_inputs['Option 0 Formula'].iloc[idx].replace('[','').replace(']','').split(',')
            ...
            aux = df_inputs['Option '+ str(option) + ' Exponent'].astype(str).str.isnumeric()
            numbers = df_inputs['Option '+ str(option) + ' Exponent'].loc[aux].values
            df_inputs['Option '+ str(option) + ' Exponent']= df_inputs['Option '+ str(option) + ' Exponent'].map(scalars_dict)
...
def build_schedule_table(plant_list, id, scheduler_table):
    ...
    for i, dfNP in enumerate(plant_list):
        ...
        scheduler_table.rename(columns={'Site Labor Hours': 'Hours run' + str(id) + '_unit' + str(i)}, inplace=True)
...
def rand_input_scaling(fname):
    ...
    for param in df_scalars.index:
        if df_scalars.loc[param, 'Distribution'] == 'lognormal':
            df_scalars.loc[param, 'New value'] = np.random.lognormal(df_scalars.at[param, 'mu/lambda'], df_scalars.at[param, 'sigma'])
        elif df_scalars.loc[param, 'Distribution'] == 'exponential':
            df_scalars.loc[param, 'New value'] = np.random.exponential(1 / df_scalars.at[param, 'mu/lambda'])
        elif df_scalars.loc[param, 'Distribution'] == 'normal':
            df_scalars.loc[param, 'New value'] = np.random.normal(df_scalars.at[param, 'mu/lambda'], df_scalars.at[param, 'sigma'])
        elif df_scalars.loc[param, 'Distribution'] == 'uniform':
            df_scalars.loc[param, 'New value'] = np.random.uniform(df_scalars.at[param, 'Min value'], df_scalars.at[param, 'Max value'])
...
def build_mc_output_series(df):
    ...
    output_cols = ['Factory Equipment Cost', 'Site Labor Cost', 'Site Material Cost', 'Total Cost']
...
def get_building_table(plant_characteristics, scaling_table):
    ...
    for account in bldg_template.index:
        if pd.isna(bldg_template.at[account, 'Area']):
            ...
            bldg_template.at[account, 'Area'] = scaling_table.at[aux, 'Scaling Factor'] * bldg_template.at[account, 'Area - PWR12']

    bldg_template['Staff Limit'] = 352.82*(bldg_template['Area'].values/1534)**0.7 # this was a data fit on workers/m^2
    
    # Scale the site prep by the reference staff limit because the above equation breaks down for open areas
    bldg_template.at['A.211.', 'Staff Limit'] = np.min([bldg_template.at['A.211.', 'Staff Limit - PWR12'], bldg_template.at['A.211.', 'Staff Limit']])
...
def run_ncet(plant, path, orders, plant_fname, param_fname, basis_fname, mc_runs=1, make_building_table=False, save_all=False):
...
if __name__ == '__main__':
    ...
    path = "C:/Users/rstew/Dropbox (MIT)/CostModel/Code_v3/uncertainty/"

    # Setup parameters:
        # param_fname: of the EXCEL sheet with two tabs: input_scaling and param_ranges
        # basis_fname: ME or BE (choosing BE starts at NOAK)
    ...
    basis_fname     = 'PWR12_ME_inflated_reduced.csv'
    ...
    run_ncet(plant, path, orders, plant_fname, param_fname, basis_fname, mc_runs=mc_runs, make_building_table=True, save_all=True)
